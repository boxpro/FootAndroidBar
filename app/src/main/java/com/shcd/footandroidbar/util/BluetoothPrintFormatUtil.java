...
//			8003     水果               1
//			---------------------------------
	public static String printCashBaData(List<PadCashRoomMessageInfo> datas,String companyName){
		StringBuffer stringBuffer = new StringBuffer();
		stringBuffer.append(formatLine());
		stringBuffer.append(formatCenter("水吧打印"));
		stringBuffer.append(formatCompanyInfo(companyName));
		for (int i = 0; i <datas.size() ; i++) {
		   PadCashRoomMessageInfo info = datas.get(i);
		   stringBuffer.append(formatItemData(info.getRoomCode(),info.getMessageContent(),info.getProductNum()));
		}
		stringBuffer.append(formatFoot());
		return stringBuffer.toString();

	}

	public static String printTeaBaData(List<PadTeaRoomMessageInfo> datas,String companyName){
		StringBuffer stringBuffer = new StringBuffer();
		stringBuffer.append(formatLine());
		stringBuffer.append(formatCenter("茶吧打印"));
		stringBuffer.append(formatCompanyInfo(companyName));
		for (int i = 0; i <datas.size() ; i++) {
			PadTeaRoomMessageInfo info = datas.get(i);
			stringBuffer.append(formatItemData(info.getRoomCode(),info.getMessageContent(),info.getProductNum()));
		}
		stringBuffer.append(formatFoot());
		return stringBuffer.toString();
	 ...
	 * @return
     */
	private static String formatCenter(String content){
		StringBuffer sb = new StringBuffer();
		for (int i = 0; i < (LINE_BYTE_SIZE - getBytesLength(content)) / 2; i++) {
			sb.append(" ");
		}
		sb.append(content+"\n");
		return sb.toString();
...
//	门店：创度软件
//	打印时间：2016-06-24 12:12:12
	private static String formatCompanyInfo(String companyName){
		StringBuffer sb = new StringBuffer();
		Date date = new Date();
		DateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		sb.append("门店："+companyName+"\n");
		sb.append("打印时间："+dateFormat.format(date)+"\n");
	...
	private static String formatItemData(String roomCodeStr,String messageStr,String productNumStr){
		StringBuffer stringBuffer = new StringBuffer();
		int roomlength =	roomCodeStr.length();
		int messageStrlength =	messageStr.length();
		stringBuffer.append(roomCodeStr);
		int t1 = T1-roomlength;
		for (int j = 0; j <t1 ; j++) {
			stringBuffer.append(" ");
		}
		stringBuffer.append(messageStr);
		int t2 = T2-messageStrlength*2;
		for (int x = 0; x <t2 ; x++) {
			stringBuffer.append(" ");
		}
		stringBuffer.append(productNumStr+"\n");
		return stringBuffer.toString();
	}

	public static String formatFoot(){
		StringBuffer stringBuffer = new StringBuffer();
		stringBuffer.append(formatLine());
		return stringBuffer.toString();
	}
